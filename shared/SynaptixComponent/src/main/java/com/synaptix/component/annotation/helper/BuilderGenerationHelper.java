 ...
 * </p>
 * 
 * Instead of making the desired object directly, the client calls a constructor (or static factory) with all of the required parameters and gets a builder object. Then the client calls setter-like
 * methods on the builder object to set each optional parameter of interest. Finally, the client calls a parameterless build method to generate the object, which is immutable. The builder is a static
 * member class of the class it builds. This class offers a method to generate a builder like this.
 * 
...
public class BuilderGenerationHelper extends ClassGenerationHelper {
	 ...
	 * @param fieldsToBePassedAndAssigned
	 *            the fields
	 */
	 ...
	 * 
	 * @param builderObjectSimpleClassName
	 *            the object class name
	 */
	...
	protected void generateBuilderInvocationMethod(VariableElement fieldElement) {
		println();
		println("  public {0} {1}({2}) {", SIMPLE_CLASS_NAME, fieldElement.getSimpleName(), manufactureField(fieldElement));
