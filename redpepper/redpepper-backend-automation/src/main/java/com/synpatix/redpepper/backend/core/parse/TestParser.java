...
public class TestParser {
		...
		for (Block block : blocks) {
			if (BlockType.COMMENT.equals(block.getBlockType())) {
				...
				if (isTextEqual(testKind, "scenario")) {
					...
					if (isTextEqual(type, "configure entity")) {
					...
					} else if (isTextEqual(testKind, "web page")) {
						...
						for (BlockLine line : block.lines) {
							if (block.lines.indexOf(line) >= 3) {
								...
								cLine.setPosition(Integer.valueOf(line.getCellAt(4)));
	...
	public TestPage readString(String string, StringBuilder htmlReport, String fileName) {
		InputStream is = new ByteArrayInputStream(string.getBytes());
		BufferedReader br = null;
		br = new BufferedReader(new InputStreamReader(is));

		return readBufferedReader(br, fileName);
	}

	public TestPage readBufferedReader(BufferedReader br, String fileName) {
		TestPage testPage = new TestPage();
