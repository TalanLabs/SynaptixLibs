...
public class SynaptixStreamStartHandler extends StreamStartHandler {
		...
		if (clientCall) {
			...
			if (toValue != null) {
				try {
					...
					return new ResponseStanzaContainerImpl(ServerErrorResponses.getStreamError(StreamErrorCondition.IMPROPER_ADDRESSING, sessionContext.getXMLLang(),
			...
			responseStanza = new SynaptixServerResponses().getStreamOpenerForClient(sessionContext.getServerJID(), responseVersion, sessionContext);
		} else if (serverCall) {
			...
			if (fromValue != null) {
				try {
					...
					return new ResponseStanzaContainerImpl(ServerErrorResponses.getStreamError(StreamErrorCondition.INVALID_FROM, sessionContext.getXMLLang(),
							"could not parse incoming stanza's FROM attribute", null));
				}
			}

			responseStanza = new SynaptixServerResponses().getStreamOpenerForServerAcceptor(sessionContext.getServerJID(), responseVersion, sessionContext,
					serverRuntimeContext.getSslContext() != null);
		} else {
			String descriptiveText = "one of the two namespaces must be present: " + NamespaceURIs.JABBER_CLIENT + " or " + NamespaceURIs.JABBER_SERVER;
