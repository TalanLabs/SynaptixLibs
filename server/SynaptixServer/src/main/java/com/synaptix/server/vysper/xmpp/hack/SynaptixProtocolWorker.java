...
import com.synaptix.server.vysper.xmpp.hack.protocol.worker.SynaptixAuthenticatedProtocolWorker;
import com.synaptix.server.vysper.xmpp.hack.protocol.worker.SynaptixEncryptedProtocolWorker;
import com.synaptix.server.vysper.xmpp.hack.protocol.worker.SynaptixInitiatedProtocolWorker;
import com.synaptix.server.vysper.xmpp.hack.protocol.worker.SynaptixStartedProtocolWorker;
import com.synaptix.server.vysper.xmpp.hack.protocol.worker.SynaptixUnconnectedProtocolWorker;

public class SynaptixProtocolWorker implements StanzaProcessor {

	final Logger logger = LoggerFactory.getLogger(SynaptixProtocolWorker.class);
	...
	public SynaptixProtocolWorker() {

		stateWorker.put(SessionState.UNCONNECTED, new SynaptixUnconnectedProtocolWorker());
		stateWorker.put(SessionState.INITIATED, new SynaptixInitiatedProtocolWorker());
		stateWorker.put(SessionState.STARTED, new SynaptixStartedProtocolWorker());
		stateWorker.put(SessionState.ENCRYPTION_STARTED, new EncryptionStartedProtocolWorker());
		stateWorker.put(SessionState.ENCRYPTED, new SynaptixEncryptedProtocolWorker());
		stateWorker.put(SessionState.AUTHENTICATED, new SynaptixAuthenticatedProtocolWorker());
	...
	/**
	 * executes the handler for a stanza, handles Protocol exceptions. also writes a response, if the handler implements ResponseStanzaContainer
	 * 
	...
	public void processStanza(ServerRuntimeContext serverRuntimeContext, SessionContext sessionContext, Stanza stanza, SessionStateHolder sessionStateHolder) {
		...
		if (stanzaHandler instanceof IStateStanzaHandler) {
			...
			if (!stateStanzaHandler.checkState(sessionContext, sessionStateHolder, stanza)) {
			...
			if (sessionStateHolder.getState() != SessionState.AUTHENTICATED) {
				...
				if (XMPPCoreStanza.getWrapper(stanza) != null && !(stanzaHandler instanceof InBandRegistrationHandler)) {
