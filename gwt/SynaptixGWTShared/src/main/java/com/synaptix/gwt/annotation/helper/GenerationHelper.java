...
package com.synaptix.gwt.annotation.helper;
...
public class GenerationHelper implements Closeable {
	...
	public void generateConstantFieldDeclaration(VariableElement fieldElement) {
		if (isConstant(fieldElement)) {
			...
			if (constantValue != null) {
				println("  {0}{1} {2} = {3};", generateModifierList(fieldElement.getModifiers().toArray(new Modifier[] {})), fieldElement.asType().toString(), fieldElement.getSimpleName(),
						constantValue);
			} else {
				println("  {0}{1} {2};", generateModifierList(fieldElement.getModifiers().toArray(new Modifier[] {})), fieldElement.asType().toString(), fieldElement.getSimpleName());
			}
		}
	}
	...
	public void generateFieldDeclaration(VariableElement fieldElement, Modifier... modifiers) {
		println("  {0}{1} {2};", generateModifierList(modifiers), fieldElement.asType().toString(), fieldElement.getSimpleName());
	}
	 ...
	 * 
	 * <pre>
	 * <code>generateFieldList(myList, true, false)</code>
	 * </pre>
	 * 
	 ...
	 * 
	 * <pre>
	 * <code>generateImports({@link EventHandler}.class, {@link GwtEvent}.class, null, {@link GenEventProcessor}.class)</code>
	 * </pre>
	 * 
	 * <b>Generated example:</b>
	 * 
	 ...
	 * </pre>
	 * 
	 * TODO: It seems as the compiler can't find GWTP classes during generation
	 * - why?
	 * 
	 * @param imports
	 *            array of classes to be imported
	 */
	...
	/**
	 * Note that to have a constant value, a field's type must be either a
	 * primitive type or String otherwise the value is null.
	 */
	 ...
	 */
	public String determineWrapperClass(TypeMirror type) {
		String typeName = type.toString();
		if (typeName.equals("byte")) {
