...
public class ClassGenerationHelper extends GenerationHelper {
	 ...
	 *            array of interface names to be implemented
	 */
	...
	public void generateInterfaceHeader(String className, Set<Modifier> modifiers, String... extraInterfaces) {
		println();
		print("{0}interface {1}", generateModifierList(modifiers.toArray(new Modifier[] {})), className);
		if (extraInterfaces != null && extraInterfaces.length > 0) {
	...
	public void generateConstructorUsingFields(String simpleClassName, Collection<VariableElement> fieldsToBePassedAndAssigned, Modifier... constructorModifiers) {
		println();
		println("  {0}{1}({2}) {", generateModifierList(constructorModifiers), simpleClassName, generateFieldList(fieldsToBePassedAndAssigned, true, false));
