 ...
 * 
 */
public final class DefaultPercentMinMaxPerimeterWidget extends AbstractMinMaxPerimeterWidget<Double> implements IPerimeterWidget {

	private static final long serialVersionUID = -7081530996734745572L;

	public DefaultPercentMinMaxPerimeterWidget(String title) {
		super(title);
	}
	...
	protected boolean isCoherent() {
		if ((hasMinComplete() && getMin() < 0) || (hasMaxComplete() && getMax() > 100) || (hasMinComplete()) && (hasMaxComplete()) && (getMin() > getMax())) {
			return false;
		}
		return true;
	}
