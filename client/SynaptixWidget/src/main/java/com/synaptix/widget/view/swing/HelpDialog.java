...
public class HelpDialog extends JPanel {
	...
	public HelpDialog(String subtitle) {
	...
	private JPanel buildContents() {
		...
		builder.add(buildMainContentPanel(), cc.xy(1, 3));

		return builder.getPanel();
	}

	/**
	 * Build the content of the dialog (does not include the buttons)
	 * 
	 * @return
	 */
	protected JComponent buildMainContentPanel() {
		return new JScrollPane(rapportPane);
	}

	protected final void setHtmlText(String text) {
		rapportPane.setText(text);
	}

	public int showDialog(Component parent, String title, String html) {
		returnValue = CLOSE_OPTION;

		rapportPane.setText(html);
	...
	private final class CloseAction extends AbstractCloseAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (((m & ActionEvent.CTRL_MASK) == ActionEvent.CTRL_MASK) && ((m & ActionEvent.SHIFT_MASK) == ActionEvent.SHIFT_MASK) && ((m & ActionEvent.ALT_MASK) == ActionEvent.ALT_MASK)) {
				new SynaptixDialog().showDialog(HelpDialog.this);
			}
