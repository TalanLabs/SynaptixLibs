...
class LazyActionMap extends ActionMapUIResource {
	...
	private void loadIfNecessary() {
		if (_loader != null) {
			...
			try {
				Method method = klass.getDeclaredMethod("loadActionMap", //$NON-NLS-1$
						new Class[] { LazyActionMap.class });
				method.invoke(klass, new Object[] { this });
			} catch (NoSuchMethodException nsme) {
				assert false : "LazyActionMap unable to load actions " + klass; //$NON-NLS-1$
			} catch (IllegalAccessException iae) {
				assert false : "LazyActionMap unable to load actions " + iae; //$NON-NLS-1$
			} catch (InvocationTargetException ite) {
				assert false : "LazyActionMap unable to load actions " + ite; //$NON-NLS-1$
