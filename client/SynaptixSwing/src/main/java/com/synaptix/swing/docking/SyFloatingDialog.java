...
public class SyFloatingDialog extends JDialog implements
		FloatingDockableContainer {

	private static final long serialVersionUID = -7673660592051164315L;
	...
	private Color controlColor = UIManager.getColor("control"); //$NON-NLS-1$
	private Color highlight = UIManager.getColor("controlLtHighlight"); //$NON-NLS-1$
	private Color shadow = UIManager.getColor("controlShadow"); //$NON-NLS-1$

	private Color activeCaptionColor = UIManager.getColor("activeCaption"); //$NON-NLS-1$
	private Color inactiveCaptionColor = UIManager.getColor("inactiveCaption"); //$NON-NLS-1$
	private Color activeCaptionBorderColor = UIManager
			.getColor("activeCaptionBorder"); //$NON-NLS-1$
	private Color inactiveCaptionBorderColor = UIManager
			.getColor("inactiveCaptionBorder"); //$NON-NLS-1$
	...
	public SyFloatingDialog(Dialog parent) {
		super(parent);
		init();
	}
	
	public SyFloatingDialog(Frame parent) {
		super(parent);
		...
		addWindowListener(new WindowAdapter() {
			...
			public void windowClosing(WindowEvent e) {
				...
				for(DockableState dockState : dockStates) {
					...
					while(c != null && !c.equals(SyFloatingDialog.this)) {
						c = c.getParent();
					}
					if (c != null) {
	...
	public void installBorders() {
		Border border = UIManager.getBorder("FloatingDialog.dialogBorder"); //$NON-NLS-1$
		Border titleBorder = UIManager.getBorder("FloatingDialog.titleBorder"); //$NON-NLS-1$
	...
	public void installResizers() { // 2005/10/06
		...
		Color inactive = UIManager.getColor("inactiveCaption"); //$NON-NLS-1$
	...
	private class ResizeListener implements MouseMotionListener, MouseListener {
		...
		public void mousePressed(MouseEvent e) {
			dragType = 0;
			Point p = SwingUtilities.convertPoint(e.getComponent(), e
					.getPoint(), getContentPane());
			/*
