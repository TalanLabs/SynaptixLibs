...
public class OldBasicPathUI extends PathUI {
			...
			for (int i = 0; i < model.getNodeCount(); i++) {
				...
				if (i < model.getNodeCount() - 1) {
					...
					if (text != null && !text.isEmpty()) {
						...
						// + (lineSize.width - ir
						// .getRectangle().width)
						// / 2, y + lineSize.height / 2
						// + 5, ir.getRectangle().width,
				...
				int w = 0;
				for (String l : lignes) {
					ImageRect ir = textImageFactory.getImageRect(l, path
				...
				for (String l : lignes) {
					ImageRect ir = textImageFactory.getImageRect(l, path
							...
							.prepareNodeColorText(renderer, i), null,
							-Math.PI / 4.0);
					g2.drawImage(ir.getImage(), x1, y - mh
							- ir.getRectangle().height, null);

//					g2.setColor(Color.RED);
//					g2.drawRect(x1, y - mh - ir.getRectangle().height, ir
//							.getRectangle().width, ir.getRectangle().height);
