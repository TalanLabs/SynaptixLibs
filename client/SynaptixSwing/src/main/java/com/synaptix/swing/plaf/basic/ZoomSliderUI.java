...
public class ZoomSliderUI extends SliderUI {
	// Old actions forward to an instance of this.
	private static final Actions SHARED_ACTION = new Actions();

	private static Image IMAGE_ZOOM_SLIDER_NORM;

	private static Image IMAGE_ZOOM_BAR_NORM;

	static {
		Image image = null;
		try {
			image = ImageIO.read(ZoomSliderUI.class
					.getResource("/images/zoom_slider_norm.png")); //$NON-NLS-1$
		} catch (IOException e) {
			e.printStackTrace();
		}
		IMAGE_ZOOM_SLIDER_NORM = image;

		try {
			image = ImageIO.read(ZoomSliderUI.class
					.getResource("/images/zoom_bar_norm.png")); //$NON-NLS-1$
		} catch (IOException e) {
			e.printStackTrace();
		}
		IMAGE_ZOOM_BAR_NORM = image;
	}
	...
	public static ComponentUI createUI(JComponent b) {
		return new ZoomSliderUI((JSlider) b);
	}
	...
	public void uninstallUI(JComponent c) {
		if (c != slider)
			throw new IllegalComponentStateException(this
					+ " was asked to deinstall() " + c //$NON-NLS-1$
					+ " when it only knows about " + slider + "."); //$NON-NLS-1$ //$NON-NLS-2$
	...
	protected void installDefaults(JSlider slider) {
		LookAndFeel.installBorder(slider, "Slider.border"); //$NON-NLS-1$
		LookAndFeel.installColorsAndFont(slider, "Slider.background", //$NON-NLS-1$
				"Slider.foreground", "Slider.font"); //$NON-NLS-1$ //$NON-NLS-2$
		highlightColor = UIManager.getColor("Slider.highlight"); //$NON-NLS-1$

		shadowColor = UIManager.getColor("Slider.shadow"); //$NON-NLS-1$
		focusColor = UIManager.getColor("Slider.focus"); //$NON-NLS-1$

		focusInsets = (Insets) UIManager.get("Slider.focusInsets"); //$NON-NLS-1$
	}
	...
	protected void installKeyboardActions(JSlider slider) {
		...
		LazyActionMap.installLazyActionMap(slider, ZoomSliderUI.class,
				"Slider.actionMap"); //$NON-NLS-1$
	}
	...
	public Dimension getPreferredHorizontalSize() {
		Dimension horizDim = (Dimension) DefaultLookup.get(slider, this,
				"Slider.horizontalSize"); //$NON-NLS-1$
		if (horizDim == null) {
	...
	public Dimension getMinimumHorizontalSize() {
		Dimension minHorizDim = (Dimension) DefaultLookup.get(slider, this,
				"Slider.minimumHorizontalSize"); //$NON-NLS-1$
		if (minHorizDim == null) {
	...
	public Dimension getMinimumVerticalSize() {
		Dimension minVertDim = (Dimension) DefaultLookup.get(slider, this,
				"Slider.minimumVerticalSize"); //$NON-NLS-1$
		if (minVertDim == null) {
	...
	public void paintTrack(Graphics g) {
		g.drawImage(IMAGE_ZOOM_SLIDER_NORM, trackRect.x, trackRect.y,
				trackRect.width, trackRect.height, null);
	}
	...
	public void paintTicks(Graphics g) {
		...
		g.setColor(DefaultLookup.getColor(slider, this, "Slider.tickColor", //$NON-NLS-1$
				Color.black));
	...
	public void paintThumb(Graphics g) {
		...
		g.drawImage(IMAGE_ZOOM_BAR_NORM, thumbRect.x, thumbRect.y,
...
//		g.translate(-knobBounds.x, -knobBounds.y);
	...
	private class Handler implements ChangeListener, ComponentListener,
		...
		public void propertyChange(PropertyChangeEvent e) {
			...
			if (propertyName == "orientation" || propertyName == "inverted" //$NON-NLS-1$ //$NON-NLS-2$
					|| propertyName == "labelTable" //$NON-NLS-1$
					|| propertyName == "majorTickSpacing" //$NON-NLS-1$
					|| propertyName == "minorTickSpacing" //$NON-NLS-1$
					|| propertyName == "paintTicks" //$NON-NLS-1$
					|| propertyName == "paintTrack" || propertyName == "font" //$NON-NLS-1$ //$NON-NLS-2$
					|| propertyName == "paintLabels") { //$NON-NLS-1$
				checkedLabelBaselines = false;
				calculateGeometry();
				slider.repaint();
			...
			} else if (propertyName == "model") { //$NON-NLS-1$
	...
	public class ActionScroller extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			SHARED_ACTION.scroll(slider, ZoomSliderUI.this, dir, block);
		}
	...
	private static class Actions extends UIAction {
		public static final String POSITIVE_UNIT_INCREMENT = "positiveUnitIncrement"; //$NON-NLS-1$
		public static final String POSITIVE_BLOCK_INCREMENT = "positiveBlockIncrement"; //$NON-NLS-1$
		public static final String NEGATIVE_UNIT_INCREMENT = "negativeUnitIncrement"; //$NON-NLS-1$
		public static final String NEGATIVE_BLOCK_INCREMENT = "negativeBlockIncrement"; //$NON-NLS-1$
		public static final String MIN_SCROLL_INCREMENT = "minScroll"; //$NON-NLS-1$
		public static final String MAX_SCROLL_INCREMENT = "maxScroll"; //$NON-NLS-1$
