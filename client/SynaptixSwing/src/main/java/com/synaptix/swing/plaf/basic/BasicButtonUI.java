...
public class BasicButtonUI extends ButtonUI {
	...
	private final static String propertyPrefix = "Button" + "."; //$NON-NLS-1$ //$NON-NLS-2$
	...
	protected void installDefaults(AbstractButton b) {
		...
		defaultTextShiftOffset = UIManager.getInt(pp + "textShiftOffset"); //$NON-NLS-1$

		// set the following defaults on the button
		if (b.isContentAreaFilled()) {
			LookAndFeel.installProperty(b, "opaque", Boolean.TRUE); //$NON-NLS-1$
		} else {
			LookAndFeel.installProperty(b, "opaque", Boolean.FALSE); //$NON-NLS-1$
		}

		if (b.getMargin() == null || (b.getMargin() instanceof UIResource)) {
			b.setMargin(UIManager.getInsets(pp + "margin")); //$NON-NLS-1$
		}

		LookAndFeel.installColorsAndFont(b, pp + "background", pp //$NON-NLS-1$
				+ "foreground", pp + "font"); //$NON-NLS-1$ //$NON-NLS-2$
		LookAndFeel.installBorder(b, pp + "border"); //$NON-NLS-1$

		Object rollover = UIManager.get(pp + "rollover"); //$NON-NLS-1$
		if (rollover != null) {
			LookAndFeel.installProperty(b, "rolloverEnabled", rollover); //$NON-NLS-1$
		}

		LookAndFeel.installProperty(b, "iconTextGap", new Integer(4)); //$NON-NLS-1$
	}
	...
	public void uninstallUI(JComponent c) {
		...
		BasicHTML.updateRenderer(c, ""); //$NON-NLS-1$
	}
	...
	public void paint(Graphics g, JComponent c) {
		...
		if (!model.isArmed() || !model.isPressed()) {
			Border border = BorderFactory.createEtchedBorder(Color.GRAY,
					Color.DARK_GRAY);
			border.paintBorder(b, g, viewRect.x, viewRect.y, viewRect.width,
					viewRect.height);
			...
			if (b.isRolloverEnabled() && model.isRollover()) {
				Border border2 = BorderFactory.createEtchedBorder(Color.YELLOW,
						Color.ORANGE);
				border2.paintBorder(b, g, viewRect.x, viewRect.y,
	...
	protected void paintButtonPressed(Graphics g, AbstractButton b) {
		...
		g.drawRoundRect(viewRect.x + 2, viewRect.y + 2, viewRect.width - 5,
				viewRect.height - 5, 5, 5);
		g.drawRoundRect(viewRect.x, viewRect.y, viewRect.width - 1,
				viewRect.height - 1, 5, 5);
	}
