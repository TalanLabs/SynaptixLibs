...
import com.synaptix.swing.roulement.JRoulementTimelineRightResourcesHeader;
import com.synaptix.swing.roulement.RoulementTimelineResource;
import com.synaptix.swing.roulement.RoulementTimelineResourceRenderer;
...
public class BasicRoulementTimelineRightResourceHeaderUI extends
		RoulementTimelineRightResourceHeaderUI {

	private static Cursor resizeCursor = Cursor
			.getPredefinedCursor(Cursor.N_RESIZE_CURSOR);

	protected JRoulementTimelineRightResourcesHeader header;
	...
	public static ComponentUI createUI(JComponent h) {
		return new BasicRoulementTimelineRightResourceHeaderUI();
	}

	public void installUI(JComponent c) {
		header = (JRoulementTimelineRightResourcesHeader) c;
	...
	private void scrollToResource(int res) {
		...
		if ((header.getParent() == null)
				...
				|| ((roulementTimeline = header.getRoulementTimeline()) == null)) {
			return;
		}
		...
		roulementTimeline.scrollRectToVisible(vis);
	}
	...
	private static boolean canResize(RoulementTimelineResource resource,
			JRoulementTimelineRightResourcesHeader header) {
		return (resource != null) && header.getResizingAllowed()
				&& resource.getResizable();
	}

	private int changeResourceHeight(
			RoulementTimelineResource resizingResource,
			JRoulementTimelineRightResourcesHeader th, int oldHeight,
			int newHeight) {
		...
		if ((th.getParent() == null)
				...
				|| ((roulementTimeline = th.getRoulementTimeline()) == null)) {
			return 0;
		}
		...
		if (!container.getComponentOrientation().isLeftToRight()
			...
			int newHeaderHeight = roulementTimeline.getHeight() + diff;

			/* Resize a table */
			Dimension tableSize = roulementTimeline.getSize();
			tableSize.height += diff;
			roulementTimeline.setSize(tableSize);
	...
	public void paint(Graphics g, JComponent c) {
		...
		RoulementTimelineResourcesModel rm = header.getResourceModel();
		...
		RoulementTimelineResource draggedResource = header.getDraggedResource();
		int resourceHeight;
	...
	private Component getHeaderRenderer(int resourceIndex, boolean isDrag) {
		RoulementTimelineResource aResource = header.getResourceModel()
		...
		JRoulementTimeline roulementTimeline = header.getRoulementTimeline();
		boolean selected = roulementTimeline.getSelectionModel()
		...
		boolean hasFocus = !header.isPaintingForPrint()
				...
				&& header.hasFocus();
		return renderer.getRoulementTimelineRightResourceRendererComponent(
				roulementTimeline, aResource, resourceIndex, selected,
				hasFocus, isDrag);
	}
	...
	private int viewIndexForResource(RoulementTimelineResource aResource) {
		RoulementTimelineResourcesModel rm = header.getResourceModel();
	...
	private int getHeaderWidth() {
		...
		RoulementTimelineResourcesModel resourceModel = header
				.getResourceModel();
	...
	public Dimension getMinimumSize(JComponent c) {
		long height = 0;
		Enumeration<RoulementTimelineResource> enumeration = header
				.getResourceModel().getResources();
		while (enumeration.hasMoreElements()) {
			RoulementTimelineResource aResource = enumeration.nextElement();
	...
	private final class MouseInputHandler implements MouseInputListener {
		...
		public void mouseClicked(MouseEvent e) {
			...
			//
			// if (index != -1) {
			// RoulementTimelineResource hitResource = resourceModel
			// .getResource(index);
			//
			// RoulementTimelineSelectionModel selectionModel = header
			// .getRoulementTimeline().getSelectionModel();
			//
			// if (e.isControlDown()) {
		...
		private RoulementTimelineResource getResizingResource(Point p) {
		...
		public void mousePressed(MouseEvent e) {
			JRoulementTimeline roulementTimeline = header
			...
			if (index != -1) {
				...
				RoulementTimelineResource resizingResource = getResizingResource(
						p, index);
		...
		public void mouseDragged(MouseEvent e) {
			...
			RoulementTimelineResource resizingResource = header
					.getResizingResource();
			RoulementTimelineResource draggedResource = header
					.getDraggedResource();
