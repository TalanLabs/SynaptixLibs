...
public class BasicSyScrollPaneUI extends SyScrollPaneUI implements ScrollPaneConstants {
	...
	protected JSyScrollPane scrollpane;
	...
	public static ComponentUI createUI(JComponent x) {
		return new BasicSyScrollPaneUI();
	}
	...
	protected void installDefaults(JSyScrollPane scrollpane) {
		LookAndFeel.installBorder(scrollpane, "ScrollPane.border"); //$NON-NLS-1$
		LookAndFeel.installColorsAndFont(scrollpane, "ScrollPane.background", //$NON-NLS-1$
				"ScrollPane.foreground", "ScrollPane.font"); //$NON-NLS-1$ //$NON-NLS-2$

		Border vpBorder = scrollpane.getViewportBorder();
		if ((vpBorder == null) || (vpBorder instanceof UIResource)) {
			vpBorder = UIManager.getBorder("ScrollPane.viewportBorder"); //$NON-NLS-1$
			scrollpane.setViewportBorder(vpBorder);
		}
		LookAndFeel.installProperty(scrollpane, "opaque", Boolean.TRUE); //$NON-NLS-1$
	}

	protected void installListeners(JSyScrollPane c) {
	...
	protected void installKeyboardActions(JSyScrollPane c) {
		InputMap inputMap = getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT);

		SwingUtilities.replaceUIInputMap(c, JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT, inputMap);

		LazyActionMap.installLazyActionMap(c, BasicScrollPaneUI.class, "ScrollPane.actionMap"); //$NON-NLS-1$
	}

	InputMap getInputMap(int condition) {
		if (condition == JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT) {
			InputMap keyMap = (InputMap) DefaultLookup.get(scrollpane, this, "ScrollPane.ancestorInputMap"); //$NON-NLS-1$
			InputMap rtlKeyMap;

			if (scrollpane.getComponentOrientation().isLeftToRight() || ((rtlKeyMap = (InputMap) DefaultLookup.get(scrollpane, this, "ScrollPane.ancestorInputMap.RightToLeft")) == null)) { //$NON-NLS-1$
				return keyMap;
			} else {
	...
	protected void uninstallKeyboardActions(JSyScrollPane c) {
	...
	protected void syncScrollPaneWithViewport() {
		...
		JViewport colFoot = scrollpane.getColumnFooter();
				...
				if (ltr) {
					...
					if (setValueCalled && ((max - currentValue) == viewPosition.x)) {
						...
						if (extent > max) {
							...
							/*
							 * The following line can't handle a small value of viewPosition.x like Integer.MIN_VALUE correctly because (max - extent - viewPositoiin.x) causes an overflow. As a
							 * result, value becomes zero. (e.g. setViewPosition(Integer.MAX_VALUE, ...) in a user program causes a overflow. Its expected value is (max - extent).) However, this seems
							 * a trivial bug and adding a fix makes this often-called method slow, so I'll leave it until someone claims.
							 */
	...
	/**
	 * MouseWheelHandler is an inner class which implements the MouseWheelListener interface. MouseWheelHandler responds to MouseWheelEvents by scrolling the JScrollPane appropriately. If the scroll
	 * pane's <code>isWheelScrollingEnabled</code> method returns false, no scrolling occurs.
	 * 
	...
	private static class Actions extends UIAction {
		private static final String SCROLL_UP = "scrollUp"; //$NON-NLS-1$

		private static final String SCROLL_DOWN = "scrollDown"; //$NON-NLS-1$

		private static final String SCROLL_HOME = "scrollHome"; //$NON-NLS-1$

		private static final String SCROLL_END = "scrollEnd"; //$NON-NLS-1$

		private static final String UNIT_SCROLL_UP = "unitScrollUp"; //$NON-NLS-1$

		private static final String UNIT_SCROLL_DOWN = "unitScrollDown"; //$NON-NLS-1$

		private static final String SCROLL_LEFT = "scrollLeft"; //$NON-NLS-1$

		private static final String SCROLL_RIGHT = "scrollRight"; //$NON-NLS-1$

		private static final String UNIT_SCROLL_LEFT = "unitScrollLeft"; //$NON-NLS-1$

		private static final String UNIT_SCROLL_RIGHT = "unitScrollRight"; //$NON-NLS-1$
	...
	class Handler implements ChangeListener, PropertyChangeListener, MouseWheelListener {
		...
		private void mouseWheelMoved(Container parent, MouseWheelEvent e) {
			if (parent != null) {
				if (parent.getMouseWheelListeners() != null) {
					for (MouseWheelListener mouseWheelListener : parent.getMouseWheelListeners()) {
						mouseWheelListener.mouseWheelMoved(e);

					}
				}
				if (!e.isConsumed()) {
					mouseWheelMoved(parent.getParent(), e);
				}
			}
		}
		...
		private void hsbStateChanged(JViewport viewport, ChangeEvent e) {
			...
			if (scrollpane.getComponentOrientation().isLeftToRight()) {
				...
				if ((extent == 0) && (value != 0) && (oldX == max)) {
					...
					/*
					 * When a pane without a horizontal scroll bar was reduced and the bar appeared, the viewport should show the right side of the view.
					 */
		...
		private void scrollPanePropertyChange(PropertyChangeEvent e) {
			...
			if (propertyName == "verticalScrollBarDisplayPolicy") { //$NON-NLS-1$
				updateScrollBarDisplayPolicy(e);
			} else if (propertyName == "horizontalScrollBarDisplayPolicy") { //$NON-NLS-1$
				updateScrollBarDisplayPolicy(e);
			} else if (propertyName == "viewport") { //$NON-NLS-1$
				updateViewport(e);
			} else if (propertyName == "rowHeader") { //$NON-NLS-1$
				updateRowHeader(e);
			} else if (propertyName == "columnHeader") { //$NON-NLS-1$
				updateColumnHeader(e);
			} else if (propertyName == "verticalScrollBar") { //$NON-NLS-1$
				updateVerticalScrollBar(e);
			} else if (propertyName == "horizontalScrollBar") { //$NON-NLS-1$
				updateHorizontalScrollBar(e);
			} else if (propertyName == "componentOrientation") { //$NON-NLS-1$
				scrollpane.revalidate();
				scrollpane.repaint();
			}
		}
		...
		private void sbPropertyChange(PropertyChangeEvent e) {
			...
			if ("model" == propertyName) { //$NON-NLS-1$
			...
			} else if ("componentOrientation" == propertyName) { //$NON-NLS-1$
